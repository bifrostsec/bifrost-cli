...
func RegisterOptions(fl *flag.FlagSet, opts *Options) {
	...
	fl.StringVar(&opts.serviceVersion, "service-version", "", "Optional version of the service; required unless --image is set")
	fl.StringVar(&opts.image, "image", "", "Optional container image reference for the uploaded SBOM (or IMAGE/BIFROST_IMAGE environment variables); required unless --service-version is set")
